...
class TestNetworkTargetResolution:
    ...
    @pytest.mark.parametrize(
        "attribute, value", [("full_url", "http://203.0.113.5/"), ("host", "203.0.113.5")]
    )
    @pytest.mark.parametrize("receiver", ["r", "alias"])
    def test_urllib_request_destination_attribute_requires_approval(
        self, attribute, value, receiver
    ):
        code = f"import urllib.request\nr = urllib.request.Request('http://pypi.org/')\nalias = r\n{receiver}.{attribute} = {value!r}\nurllib.request.urlopen(r)"
        assert is_high_risk_tool_call("python", {"code": code}) is True

    @pytest.mark.parametrize("attribute", ["full_url", "host"])
    def test_unused_urllib_request_destination_write_does_not_prompt(self, attribute):
    ...
    @pytest.mark.parametrize("tail", ["", "def unused():\n    configure()"])
    def test_uncalled_helper_does_not_change_session_proxy(self, tail):
        code = (
            "import requests\ns = requests.Session()\ns.trust_env = False\ndef configure():\n    s.proxies = {'https': 'http://203.0.113.5/'}\n"
            + tail
            + "\ns.get('https://pypi.org/')"
    ...
    @pytest.mark.parametrize(
        ...
        [
            "configure()",
            "alias = configure\nalias()",
            "list(map(lambda _: configure(), [0]))",
            "def invoke():\n    configure()\ninvoke()",
            "globals()['configure']()",
        ],
    )
    def test_reachable_helper_retains_session_proxy(self, invocation):
        code = (
            "import requests\ns = requests.Session()\ndef configure():\n    s.proxies = {'https': 'http://203.0.113.5/'}\n"
            + invocation
            + "\ns.get('https://pypi.org/')"
    ...
    @pytest.mark.parametrize("tail", ["configure()", "def invoke():\n    configure()\ninvoke()"])
    def test_later_helper_call_does_not_change_earlier_request(self, tail):
        code = (
            "import requests\ns = requests.Session()\ndef configure():\n    s.proxies = {'https': 'http://203.0.113.5/'}\ns.get('https://pypi.org/')\n"
            + tail
        )
    ...
    @pytest.mark.parametrize("receiver", ["s.proxies", "mapping"])
    def test_deleted_proxy_does_not_block_request(self, receiver):
        code = (
            "import requests\ns = requests.Session()\ns.trust_env = False\ns.proxies = {'https': 'http://203.0.113.5/'}\nmapping = s.proxies\n"
            + f"del {receiver}['https']\ns.get('https://pypi.org/')"
        )
        assert _check_code_safety(code) is None
        assert is_high_risk_tool_call("python", {"code": code}) is False

    @pytest.mark.parametrize(
        "removal", ["if False:\n    del s.proxies['https']", "del s.proxies['http']"]
    )
    def test_proxy_deletion_preserves_other_destinations(self, removal):
        code = (
            "import requests\ns = requests.Session()\ns.proxies = {'https': 'http://203.0.113.5/', 'http': 'https://pypi.org/'}\n"
            + removal
            + "\ns.get('https://pypi.org/')"
